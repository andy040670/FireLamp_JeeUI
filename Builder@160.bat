...
Echo  # Build Esp8266@160                 - ([97m2[92m)#
...
Echo  # Build and upload Esp8266@160      - ([97m6[92m)#
...
if "%choice%"=="2" ("%USERPROFILE%\.platformio\penv\Scripts\pio.exe" run --environment esp8266@160)
if "%choice%"=="3" ("%USERPROFILE%\.platformio\penv\Scripts\pio.exe" run --target buildfs --environment esp8266@160)
if "%choice%"=="4" (curl -v http://192.168.1.70/update -F "name=@.pio\build\esp8266@160\firmware.bin")
if "%choice%"=="5" (curl -v http://192.168.1.70/update -F "name=@.pio\build\esp8266@160\littlefs.bin")
if "%choice%"=="6" ("%USERPROFILE%\.platformio\penv\Scripts\pio.exe" run --target upload --environment esp8266@160)
...
if "%choice%"=="8" ("%USERPROFILE%\.platformio\penv\Scripts\pio.exe" run --target erase --environment esp8266@160)
if "%choice%"=="9" (
	pio system prune -f
