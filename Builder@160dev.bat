...
:m1
Echo  #****************************************#
Echo  #     Commands [91m[env:esp8266@160dev][30m      #
Echo  #----------------------------------------#
...
Echo  # Build and upload Esp8266@160dev   - (6)#
...
if "%choice%"=="2" ("%USERPROFILE%\.platformio\penv\Scripts\pio.exe" run --environment esp8266@160dev)
if "%choice%"=="3" ("%USERPROFILE%\.platformio\penv\Scripts\pio.exe" run --target buildfs --environment esp8266@160dev)
if "%choice%"=="4" (curl -v http://192.168.1.70/update -F "name=@.pio\build\esp8266@160dev\firmware.bin")
if "%choice%"=="5" (curl -v http://192.168.1.70/update -F "name=@.pio\build\esp8266@160dev\littlefs.bin")
if "%choice%"=="6" ("%USERPROFILE%\.platformio\penv\Scripts\pio.exe" run --target upload --environment esp8266@160dev)
if "%choice%"=="7" ("%USERPROFILE%\.platformio\penv\Scripts\pio.exe" run --target uploadfs --environment esp8266@160dev)
if "%choice%"=="p" (start %workdir%\SerialMonitor.cmd)
if "%choice%"=="8" ("%USERPROFILE%\.platformio\penv\Scripts\pio.exe" run --target erase --environment esp8266@160dev)
if "%choice%"=="9" (
	pio system prune -f
